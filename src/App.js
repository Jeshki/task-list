  ...
  return (
    <div className='app'>
      <div className="container">
        <h1><DiReact className='logo' /> Darbaraštis</h1>
        ...
        <div>
          {tasks.map(task => (
            <div className={`task-row ${task.completed ? 'įvykdytas' : ''}`} key={task.id} onDoubleClick={() => toggleComplete(task.id)} >
              <p>{task.text} </p>
          ...
          <p className='length'>{(tasks < 1) ? 'Darbų nėra' : `Tasks: ${tasks.length}`}</p>
      </div>
    </div>
  );
}
